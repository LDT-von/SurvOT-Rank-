...
1. **DCT v3.3：当前第一主线。** 仓库内分数最高，目标函数也已经简化到两个核心项；论文必须把贡献限定为“删失感知的分布敏感性建模”，不能声称因果反事实。
2. **CA-PSA：第二主线。** 叙事比 DCT 更整洁，但和 SlotSPE、AdaSlot、BO-QSA、Dual-State Slot Attention、SurvQ 存在明显邻近，必须用“队列共享身份 + 患者状态 + 生存监督下自适应激活”的统一机制守住边界。
...
|---:|---|---|---:|---:|---|---|---|---|
| 1 | **DCT v3.3 score-first** | BLCA best `0.7311±0.0293`；last5 `0.6453±0.0706`；BRCA best `0.6886±0.0382` | 2 | 7.5/10 | 中 | Q3；证据补强后可试 Q2 | Q1/Q2；CCF B 有条件，CCF A 不现实 | 第一主线 |
| 2 | **CA-PSA** | 服务器暂定 5-fold best mean `0.7217`，尚未纳入主结果档案 | 3 | 7.0/10 | 中高 | Q3；当前不宜声称完成论文证据 | Q1/Q2；CCF B/C | 第二主线 |
...
- DCT 新增 BRCA 结果为 best `0.6886±0.0382`、last5 `0.5001±0.0541`，说明当前配方在样本更大、异质性更强的癌种上存在严重后期过拟合；这降低了跨癌种泛化证据，但没有改变 BLCA 主线排序。
...
**致命审稿问题。** 最高分主要来自 score-first 目标，而不是原始反事实 transport 训练项。审稿人会问：如果所有 DCT 特有损失均关闭，提升究竟来自 DCT 表征、IPCW ranking，还是普通容量/训练技巧？这必须靠结构消融回答。
...
**最接近工作与边界。** [MOTCat](https://arxiv.org/abs/2306.08330) 已把 OT 用于 WSI–genomics 生存对齐；[MMP](https://openreview.net/forum?id=3MfvxH3Gia) 已使用形态/通路 prototypes 与跨模态对齐；删失生存的 learning-to-rank 也不是新问题（例如 [Learning to Rank for Censored Survival Data](https://arxiv.org/abs/1806.01984)）。2026 年的 [CURE](https://arxiv.org/abs/2602.19987) 已直接使用“multimodal counterfactual time-to-event”表述。因此 DCT 不能把“prototype”“OT”“ranking”或“counterfactual survival”单独当创新点，只能主张这些组件在**训练折删失估计下的分布敏感性分析机制**上的统一设计。

**风险。** 撞题中等；过度宣称风险高。`counterfactual` 必须定义为 model-based intervention/sensitivity，不能写成治疗因果效应，也不能暗示可识别的个体反事实生存时间。
...
**投稿判断。** 暂定 BLCA best mean `0.7217` 有竞争力，但尚不足以独立成文。补齐十癌种、身份稳定性和动态 slot 的必要性验证后，合理目标为 Q1/Q2 或 CCF B/C。若只给 BLCA C-index 和常规消融，最多 Q3/Q2 边缘。
