...
class OTEHTimeLocalCompeting(OTEHV2RankEvent):
    """V50: Time-Localized Optimal Transport Event Decomposition（时间局部竞争事件
    危险率分解）。

    在 V45（`OTEHV2RankEvent`）的 OT 诱导事件骨架之上做两个结构性改动（不是已知
    trick 的堆叠，而是改变了 事件 -> hazard 的建模机制）：

    创新 A —— 时间局部事件头（Time-local event heads）
        不再让每个事件预测一整条完整 hazard 曲线，而是给每个事件在“每个离散生存
        时间 bin”上一个可学习责任权重；对每个时间 bin，事件之间相互竞争
        （softmax over events），从而自发分化出“早期风险事件 / 中期事件 / 晚期
        事件”。配套两个正则：
        - 时间特化（specialization）：每个事件在时间轴上的责任分布应尖锐（低熵）。
        - 时间覆盖（coverage）：所有事件的总责任应铺满时间轴（高熵），避免挤在
          同一 bin。

    创新 B —— 竞争性风险/保护门控（Competing risk/protective gate）
        事件拆成“风险增强”和“风险保护”两条通路，各自有独立 hazard 头和时间局部
        门控。最终 hazard = 风险贡献 - beta * 保护贡献（beta 可学习、softplus
        非负）。这让模型能表达“某些事件降低风险”，医学上对应保护性因素；配一个
        小 L2 稳定项防止保护通路发散。

    本类原为独立文件夹 `50_otehv2_timelocal_competing/` 中的游离实现（单独训练、
    单独出结果），现合并进主项目并接入 `METHOD_REGISTRY`（方法名
    `otehv2_timelocal_competing`，别名 `50`）统一管理。迁移后逐位保留原实现的
    数值行为，只是改为直接继承本文件内的 `OTEHV2RankEvent`（而不是通过独立的
    `model_v45.py`/`backbone.py` 拷贝）。
    """
...
__all__ = ["OTEHV2RankEvent", "OTEHV2RankEventV2", "OTEHTimeLocalCompeting"]
