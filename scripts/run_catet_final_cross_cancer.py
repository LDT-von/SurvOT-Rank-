#!/usr/bin/env python3
"""Run the frozen CATET repaired recipe across feature-complete cancers.
...
RESULT_ROOT = Path("results/catet_repaired_final")
...
def build_job(args: argparse.Namespace, cancer: str, fold: int, *, smoke: bool) -> Job:
    config = Path("configs") / f"censoring_aware_temporal_evidence_transport_{cancer}.yaml"
    ...
    values.update({
        ...
        "specific_simple": f"catet_repaired_{cancer}_50ep",
    })
    if smoke:
        values.update({"max_epochs": 2, "max_smoke_batches": 2})
        result_dir = Path("results/catet_final_smoke") / cancer
        values["results_dir"] = result_dir.as_posix()
        values["specific_simple"] = f"catet_smoke_{cancer}"
    command = (
...
def scheduler_lock_path(gpu: str, smoke: bool) -> Path:
    ...
    return Path("results/catet_final_queue") / f".{kind}_gpu_{_safe_gpu_name(gpu)}.lock"


def task_lock_path(job: Job) -> Path:
    return job.result_dir / f".split_{job.fold}.catet_final.lock"


def doctor(args: argparse.Namespace) -> int:
    failed = False
    for cancer in args.cancers:
        config = REPO_ROOT / "configs" / f"censoring_aware_temporal_evidence_transport_{cancer}.yaml"
...
def print_plan(jobs: list[Job], *, force: bool = False, run_mode: bool = False) -> None:
    print("FINAL METHOD: CATET repaired (frozen, 4 stages, monotone epsilon)")
...
def run_queue(args: argparse.Namespace, jobs: list[Job], *, smoke: bool) -> int:
    ...
    try:
        scheduler_lock = acquire_run_lock(
            ...
            label=f"final CATET cross-cancer queue on GPU {args.gpu}",
        )
    ...
    try:
        for index, job in enumerate(jobs, start=1):
            ...
            task_lock = None
            try:
                task_lock = acquire_run_lock(task_lock_path(job), label=f"CATET {job.cancer.upper()} fold{job.fold}")
            except ActiveRunError as error:
                print(f"[skip-running] {error}")
                continue
            try:
                print(f"\n[{index:02d}/{len(jobs):02d}] CATET {job.cancer.upper()} fold{job.fold}")
                print(shlex.join(job.command))
