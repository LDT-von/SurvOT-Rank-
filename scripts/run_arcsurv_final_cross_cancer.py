#!/usr/bin/env python3
"""Run the frozen ArcSurv staged recipe across feature-complete cancers.

This launcher replicates the OLD staged behavior (NOT the failed hard-repair)
by explicitly setting arc_anchor_logit=0.0, arc_lambda_sharpness=0.0, and
arc_bank_update_epochs=1.  The purpose is to evaluate one paper-facing method
under one protocol, not to tune per cancer.
"""
...
RESULT_ROOT = Path("results/arcsurv_staged_final")
WHICH_SPLITS = "5fold_uni2h"

# Paper-facing frozen recipe: old staged behavior WITHOUT hard-repair.
# arc_anchor_logit=0.0  -> no furthest-point anchoring (old random beta init)
# arc_lambda_sharpness=0.0 -> no per-patient entropy penalty
# arc_bank_update_epochs=1 -> freeze memory after epoch 1 (old behavior)
FINAL_OVERRIDES: dict[str, object] = {
    ...
    "arc_rank_margin": 0.0,
    "arc_rank_max_pairs": 4096,
    "arc_warmup_epochs": 5,
    "arc_ramp_epochs": 10,
    "arc_bank_update_epochs": 1,
    # ── explicitly disable hard-repair ──
    "arc_anchor_logit": 0.0,
...
def build_job(args: argparse.Namespace, cancer: str, fold: int, *, smoke: bool) -> Job:
    config = Path("configs") / f"archetypal_risk_composition_{cancer}.yaml"
    ...
    values.update({
        ...
        "specific_simple": f"arcsurv_staged_{cancer}_50ep",
    })
    if smoke:
        values.update({"max_epochs": 2, "max_smoke_batches": 2})
        result_dir = Path("results/arcsurv_final_smoke") / cancer
        values["results_dir"] = result_dir.as_posix()
        values["specific_simple"] = f"arcsurv_smoke_{cancer}"
    command = (
...
def scheduler_lock_path(gpu: str, smoke: bool) -> Path:
    ...
    return Path("results/arcsurv_final_queue") / f".{kind}_gpu_{_safe_gpu_name(gpu)}.lock"


def task_lock_path(job: Job) -> Path:
    return job.result_dir / f".split_{job.fold}.arcsurv_final.lock"


def doctor(args: argparse.Namespace) -> int:
    failed = False
    for cancer in args.cancers:
        config = REPO_ROOT / "configs" / f"archetypal_risk_composition_{cancer}.yaml"
...
def print_plan(jobs: list[Job], *, force: bool = False, run_mode: bool = False) -> None:
    print("FINAL METHOD: ArcSurv staged (frozen, 6 archetypes, NO hard-repair)")
...
def run_queue(args: argparse.Namespace, jobs: list[Job], *, smoke: bool) -> int:
    ...
    try:
        scheduler_lock = acquire_run_lock(
            ...
            label=f"final ArcSurv cross-cancer queue on GPU {args.gpu}",
        )
    ...
    try:
        for index, job in enumerate(jobs, start=1):
            ...
            task_lock = None
            try:
                task_lock = acquire_run_lock(task_lock_path(job), label=f"ArcSurv {job.cancer.upper()} fold{job.fold}")
            except ActiveRunError as error:
                print(f"[skip-running] {error}")
                continue
            try:
                print(f"\n[{index:02d}/{len(jobs):02d}] ArcSurv {job.cancer.upper()} fold{job.fold}")
                print(shlex.join(job.command))
