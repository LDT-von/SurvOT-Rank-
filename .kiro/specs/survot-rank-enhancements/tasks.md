...
- [x] 1. Slot Attention V2 —— Identity/State 解耦（需求4）
  ...
  - [x]* 1.2 为 identity/state 解耦编写单元测试（`tests/test_slot_attention_v2.py`）
...
- [x] 2. Slot Attention V2 —— Sinkhorn 路由机制（需求5a）
  ...
  - [x]* 2.2 为 Sinkhorn 路由编写单元测试（追加到 `tests/test_slot_attention_v2.py`）
...
- [x] 3. Slot Attention V2 —— 跨模态条件化更新（需求5b）
  ...
  - [x]* 3.2 为跨模态条件化编写单元测试（追加到 `tests/test_slot_attention_v2.py`）
...
- [x] 4. Slot Attention V2 —— 自适应迭代次数与工厂函数（需求5c + 5.8 + 7.1）
  ...
  - [x]* 4.2 为自适应迭代次数与工厂函数编写单元测试（追加到 `tests/test_slot_attention_v2.py`）
...
- [x] 5. Checkpoint —— 确保 Step A（Slot Attention V2）所有测试通过
...
- [x] 6. 统一生存目标函数 UnifiedSurvivalObjective（需求3）
  ...
  - [x]* 6.2 为 UnifiedSurvivalObjective 编写单元测试（`tests/test_unified_objective.py`）
...
- [x] 7. Checkpoint —— 确保 Step B（UnifiedSurvivalObjective）所有测试通过
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 8. OTEHV2RankEventV2 骨架、方法注册与配置校验（需求6.3-6.5/6.8，需求7.1-7.4）
  ...
  - [x]* 8.4 为骨架子类编写单元测试（`tests/test_otehv2_rankevent_v2.py`）
...
- [x] 9. 临床编码器与三模态融合接入（需求1）
  ...
  - [x]* 9.2 为 ClinicalEncoder 编写单元测试（`tests/test_clinical_encoder.py`）
  ...
  - [x]* 9.5 为 Clinical 分支与三模态融合编写单元测试（追加到
...
- [x] 10. Checkpoint —— 确保 Step C（临床模态融合）所有测试通过
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 11. 跨能力集成：路由重设计与统一目标接入 OTEHV2RankEventV2（需求5.4/5.9/3.6，
  ...
  - [x]* 11.3 为跨能力组合编写集成级单元测试（追加到 `tests/test_otehv2_rankevent_v2.py`）
...
- [x] 12. Checkpoint —— 确保 Step D（跨能力集成）所有测试通过
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 13. 多癌种配置文件生成（需求2.1, 2.2）
  ...
  - [x]* 13.2 为配置生成脚本编写单元测试（`tests/test_gen_multicancer_configs.py`）
...
- [x] 14. 跨癌种结果汇总工具（需求2.3-2.7）
  ...
  - [x]* 14.2 为跨癌种汇总工具编写单元测试（`tests/test_aggregate_cross_cancer.py`）
