...
| 8 | v3.8 `robust/base` BLCA 折 1/2/4 (UNI2-h, 30ep) | ⬜ **未开始** | 3 次训练 | **已核查：`robust/base` BLCA 只有 fold0 = 0.6449，折 1/2/4 无数据。** `highscore/base` 不能替代——两协议差三处：`fit_bins_on_train` (False vs True)、`which_splits` (`5fold` vs `5fold_uni2h`)、`dct_slot_init_mode` (gaussian vs deterministic)。B 组基线仍缺 |
...
| 10 | v3.3 clean 协议基线：`fit_bins_on_train=true`，UNI v1，`5fold`，折 1/2/4 | ⬜ **未开始** | 3 次训练 | **新增。** 现 v3.3 用默认 leaky 分箱，与所有新方法协议不一致，导致 A 组无有效基线 |

**结算：10 项中已完成 2、部分完成 2、未开始 6。**
零成本：#7 #9。各 3 次训练：#3 #4 #8 #10。#5 视范围而定，#6 待定。

> **依赖关系**：#9 → #8/#10 → 才能谈 #3/#4；#5 应等方法收敛到 1~2 个再做，
> 否则训练次数会乘上方法数。
