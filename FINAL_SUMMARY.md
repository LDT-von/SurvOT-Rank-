...
| reg | 9 | **0.6692** | 0.4818 | 0.0350 | 0.6509 | 0.5257 | 🔄 E28/50 |
...
| reg_legacy | 3 | 0.6441 | 0.4251 | 0.2327 | 0.4524 | 0.6037 | 🔄 E28/50 |

### 关键发现

1. **qcut 分箱下 C-Index 集中在 0.59-0.67**，reg 逆势突围到 0.6692，其余变体间差异小（±0.02），BRCA 事件稀疏（~8% DSS events）是根本瓶颈
2. **reg (保守优化器) 是 qcut 最佳**：lr=2e-4, reg=1e-3，best=**0.6692 @ E9**（较 E4 的 0.6271 大幅提升），仍在训练中(E28)
...
| 4 | reg_legacy | 0.6441 | 等宽 | 🔄 E28 |
| 5 | strat_legacy | 0.6259 | 等宽 | IBS=0.22 |

> ⚠️ 13/13 fold0 epoch_curve 已生成。reg/reg_legacy 仍在训练中。fold2 全部未跑。
