...
| 8 | v3.8 `robust/base` BLCA 折 1/2/4 (UNI2-h, clean, 30ep) | ⬜ **未开始** | 3 次训练 | **已核查：`robust/base` BLCA 只有 fold0 = 0.6449，折 1/2/4 无数据。** `highscore/base` 不能替代——`fit_bins_on_train` (False vs True) 与 `dct_slot_init_mode` (gaussian vs deterministic) 均不同。UNI2-h 侧 clean 基线 |
...
| 10 | v3.3 clean 协议基线：`fit_bins_on_train=true`，UNI v1，`5fold`，折 1/2/4 | ⬜ **未开始** | 3 次训练 | 现 v3.3 用默认 leaky 分箱，与所有新方法协议不一致 → UNI v1 侧 clean 基线 |

**结算：10 项中已完成 3、部分完成 1、未开始 6。**
零成本已清空。各 3 次训练：#3 #4 #8 #10。#5 视范围而定，#6 待定。

> **依赖关系**：#9 已完成 → 现在 #10（UNI v1 clean 基线）与 #8（UNI2-h clean 基线）
> 是所有后续判定的前置，共 6 次训练。之后才能谈 #3/#4。
