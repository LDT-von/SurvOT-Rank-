#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V45 独立骨架代码（内嵌自 31_ot_event_hazard_v2/model_v2.py）。

把 V45 (OTEHV2RankEvent) 依赖的父类 OTEventHazardV2Survival 及其工具函数
直接内嵌到本文件夹，避免运行时跨文件夹动态 import 31_ot_event_hazard_v2。

这样 45_otehv2_rankevent/ 这一个文件夹即可独立运行，只依赖：
  1. 本文件夹内的 backbone.py / model.py / paths.py / args.py / train.py
  2. 外部的 SlotSPE/ 基础仓库（数据集、loss、底层网络层，所有实验共用的基座）

注意：如果未来要修改骨架结构（OT 融合方式、事件编码器等），直接改这个文件，
不会影响其他实验目录（31/44/46-50 等），也不会被它们的改动影响。
"""
...
from models.slot_attention import MultiHeadSlotAttention  # noqa
from models.omics_encoder import SNN_Block, WSI_Mlp  # noqa


# ============================================================================
# OT 代价矩阵 + log-domain Sinkhorn（数值稳定版）
# ============================================================================
def cosine_cost(x, y):
...
# ============================================================================
# 多尺度 OT 融合：三种代价矩阵 → 三个 OT 计划 → 拼接 → 跨模态注意力融合成事件 token
# ============================================================================
...
# ============================================================================
# OT 事件危险率骨架（v9 / 31_ot_event_hazard_v2 的独立复制）
# ============================================================================
class OTEventHazardV2Survival(nn.Module):
    ...
    def _encode_omics(self, kwargs):
        ...
        # RNASeq / GeneEmbedding: produce (B, dim); slot_attention expects (B, N, D)
        h = self.sig_networks(kwargs["x_omics"])
...
__all__ = [
    "cosine_cost", "euclidean_cost", "dot_cost", "log_sinkhorn_plan",
    "MultiScaleOTFusion", "OTEventHazardV2Survival",
]
