...
## 2026-07-13：全量实验审计 — 哪些有结果、哪些没有

### 概览：共 20 次实验/运行，15 次成功，5 次失败

---

### 一、有结果 ✅（15 次）

#### v45_blca — 8 损失 (config: v45_blca.yaml)

| # | 配置 | Fold | Epoch | 环境 | Fold 2 val | 5-fold mean±std | 状态 |
|---|------|------|-------|------|-----------|----------------|------|
| 1 | seed=3, alpha_surv=0.15 | 5-fold | 30 | newSlotSPE V0 basline | 0.6013 | **0.6993 ± 0.0218** | ✅ 完整 |
| 2 | seed=3, batch_20260712_1912 | 5-fold | 30 | SurvOT-Rank | 0.6013 | 0.6885 (fold 4 未完成) | ⚠️ fold4 中断 |
| 3 | seed=None | fold2 only | 30 | SurvOT-Rank | **0.6253** (best), 0.5768 (last5) | — | ✅ 完成 |

#### v45v2_blca_clinical — 8 损失 + age/gender (config: v45v2_blca_clinical.yaml)

| # | 配置 | Fold | Epoch | Fold 2 val | 5-fold mean±std | 状态 |
|---|------|------|-------|-----------|----------------|------|
| 4 | seed=3 | 5-fold | 30 | 0.6237 | **0.6919 ± 0.0499** | ✅ SUMMARY.md 已写 |

#### Rank-Guided Event Transport — 3 损失 (config: rank_guided_event_transport_blca.yaml)

| # | 配置 | Fold | Epoch | Fold 2 val | 5-fold mean±std | 状态 |
|---|------|------|-------|-----------|----------------|------|
| 5 | seed=3, 3ep smoke | fold2 only | 3 | 0.5592 | — | ✅ smoke test |
| 6 | seed=3, 25ep | fold2 only | 25 | **0.6341** (best), 0.5616 (last5) | — | ✅ train_cindex 从 0.47→0.73 |
| 7 | seed=1, 10ep + grad_clip | 5-fold | 10 | 0.6037 | **0.6307 ± 0.0205** (last5) | ✅ quick_robust_eval |
| 8 | no seed, fold2 + grad_clip | fold2 | 🔄 进行中 | — | — | 🔄 当前运行 |

#### newSlotSPE 消融 (config: v45_blca.yaml, seed=3)

| # | 配置 | 说明 | 5-fold mean±std | Fold 2 | 状态 |
|---|------|------|----------------|--------|------|
| 9 | V0 baseline | 8 损失全开 | 0.6993 ± 0.0218 | — | ✅ |
| 10 | V1 wd=1e-4+rank | 降 rankevent 权 | 0.6991 ± 0.0323 | — | ✅ |
| 11 | V2 wd=5e-4 no rank | 关 rankevent | **0.7100 ± 0.0186** | — | ✅ |
| 12 | V3 wd=1e-3 no rank | 与 V2 md5 一致 | =V2 (0.7100) | — | ✅ (wd 无效，同 V2) |
| 13 | V4a AdamW wd=5e-4 | fold3 only | — | 0.6842 (同 V2) | ✅ (wd 仍无效) |

#### robust_eval 工具

| # | 工具 | 测试 | 结果 | 状态 |
|---|------|------|------|------|
| 14 | epoch_curve_selection.py | selftest (合成数据) | optimism_gap=0.073 | ✅ |
| 15 | honest_report.py | quick_train 报告 | report.md 已生成 | ✅ |
...
| # | 工具/配置 | 失败原因 | 详情 |
|---|----------|---------|------|
| 16 | stable_train_launcher (seed=1) | `__func__` 错误 | monkey-patch `optimizer.step` 在 PyTorch 2.x 不兼容 |
| 17 | stable_train_launcher (seed=2) | `__func__` 错误 | 同上，所有 5 个 fold 均失败 |
| 18 | stable_train_launcher (seed=3) | `__func__` 错误 | 同上，seed3/seed2/seed1 全部 0 有效 epoch |
| 19 | v45 batch 20260712_1909 | 代码版本错误 | 尾空格目录的旧 buggy 代码，已清理 |
| 20 | v45 batch 20260712_1912 fold4 | 手动 kill | 诊断完成，不需要跑完 |

---

### 三、Fold 2 汇总（所有实验）

| 模型 | 损失数 | Epoch | Seed | val best | val last5 | train_cindex 能拟合? |
|------|-------|-------|------|---------|----------|-------------------|
| v45 | 8 | 30 | 3 | 0.6013 | — | ❌ (0.48) |
| v45 | 8 | 30 | None | 0.6253 | 0.5768 | ❌ (未记录) |
| v45v2+clinical | 8 | 30 | 3 | 0.6237 | — | ❌ |
| Rank-Guided | **3** | 25 | 3 | **0.6341** | 0.5616 | ✅ (0.53→**0.73**) |
| Rank-Guided | **3** | 10 | 1 | 0.6037 | 0.5955 | ✅ (0.53) |
| Rank-Guided+grad_clip | **3** | 30 | None | 🔄 | 🔄 | 🔄 当前运行 |

**结论**：3 损失让 train_cindex 从 0.47→0.73（模型能学了），但 Fold 2 val 仍然 0.60-0.63，没有显著突破。grad_clip 能否进一步改善，待当前运行结果。
