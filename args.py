...
# -*- encoding: utf-8 -*-
"""
@file: args.py（45_otehv2_rankevent 独立参数解析器）
@desc: 精简自 common/extended_args.py，只保留：
       1. SlotSPE 训练必需的基础参数（study/data/split/training/slot 等）
       2. 灵活停止训练参数（早停 + 手动中断安全落盘）
       3. V45 (otehv2_rankevent) 自身的超参数（骨架 otehv2_* + rankevent_*）

       去掉了其他 40+ 个实验方向的参数分组，本文件只服务于这一个模型。
...
def build_parser():
    ...
    parser.add_argument("--method", type=str, default="SlotSPE_otehv2_rankevent",
                         help="保留兼容字段，仅用于结果目录命名")
    parser.add_argument("--newslot_method", type=str, default="otehv2_rankevent",
...
def process_args(argv=None):
    ...
    args.newslot_method = "otehv2_rankevent"
    return args
